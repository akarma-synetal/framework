...
export const SysProjectBranch = ObjectSchema.create({
  ...
  listViews: {
    ...
    production: {
      ...
      label: 'Production',
      data: { provider: 'object', object: 'sys_project_branch' },
      columns: ['project_id', 'display_name', 'status', 'database_url', 'updated_at'],
  ...
  fields: {
    ...
    database_url: Field.text({
      ...
      required: false,
      maxLength: 2048,
      description: 'Physical connection string for this branch.',
      group: 'Storage',
    }),
    ...
    database_auth_token: Field.text({
      ...
      required: false,
      maxLength: 2048,
      description: 'Encrypted token (Turso/libSQL) when applicable.',
      group: 'Storage',
    }),
