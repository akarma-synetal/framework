 ...
 *
 * Mechanical: the malformed field `scale`/`precision` delete (#8321) — a
 * non-integer or negative digit count declared nothing #7501's enforcement
...
export const RETIRED_KEYS_BY_MAJOR: Readonly<Record<number, readonly string[]>> = {
  ...
  // Post-cut retirements: v17.0.0 was cut before these landed, so the
  // tombstone ships on the 17.x line (launch-window convention) and the
  // prescription registers at the major boundary where `migrate meta` users
  // look — the step18 discipline, applied to retired keys.
  18: [
    // One file per entry under `entries/retired-keys/`, concatenated here sorted by
    // entry id by `gen:migration-registry` (#7297). Add an entry by adding a
    // FILE — never by editing between the markers, which is generated.
...
export const RETIRED_DEFS_BY_MAJOR: Readonly<Record<number, readonly string[]>> = {
  ...
  // Post-cut retirements: v17.0.0 was cut before these landed, so the
  // removal ships on the 17.x line (launch-window convention) and the
  // prescription registers at the major boundary where `migrate meta` users
  // look — the step18 discipline the retired-key table above already applies,
  // extended to whole defs.
  18: [
    // One file per entry under `entries/retired-defs/`, concatenated here sorted by
    // entry id by `gen:migration-registry` (#7297). Add an entry by adding a
    // FILE — never by editing between the markers, which is generated.
