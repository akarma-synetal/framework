...
- d4a687a: feat(lint): the unprovisioned-anchor warning reaches filter and page-binding surfaces (#8340)
...
- 90336e6: feat(lint): report `sharingModel: controlled_by_parent` with nothing to derive access from (#7503)
...
- df95346: feat(spec,lint): gate `enable.apiMethods` ⊆ affordances at authoring time, not only in the boot log (#7521)
  ...
  **One predicate, two consumers.** The judgement moved to
  ...
  a tidy-up: a second copy of this table at either consumer would _be_ the
...
- 19bca8c: feat(lint): refuse a path-shaped right-hand side in a metadata-form predicate (#7659)
  ...
  **Two severities, on one id:**
  ...
  - **`warning` for a bare single word** (`status == active`). This one _works_
...
- ba5e957: feat(lint): `validateSecurityPosture` now gates `permission` / `book` writes at the runtime publish door; `security-role-word` splits into its own CLI-only rule (#8310)
...
- e41c1f2: feat(lint): the ADR-0091 seed pair now gates runtime `seed` publishes (#8307)
...
- 333769d: feat(lint): the `views[]` visibility-predicate family now gates runtime `view` publishes (#7220)
...
- 9d1d9c7: feat(spec): close the 31 SDUI component-props shapes against unknown keys (#4001 batch A)
...
- df72328: `visibility-root-mislayered` and `visibility-bare-identifier` now explain the metadata-editing layer by naming the **surface** — a schema-bound metadata-editing form, the row under edit — instead of a `*.form.ts` filename.
...
- 78ed1f4: fix(lint): one prescription for a bare unquoted word on the right of `==` (#7696)
  ...
  **The bare-word right-hand position now produces exactly one finding.** The two
  root-prescribing rules stand down for identifiers that occur _only_ as a bare
...
- fc87586: fix(lint): `validateSecurityPosture`'s `surfaceReason` claimed a coverage the ADR-0094 gate does not give it — 1 of 13 rules (#7576)
  ...
  - **Double-reporting.** It cannot happen, structurally rather than by luck.
    ...
    `invalid_metadata`) _before_ `runAuthoringGate` (the ADR-0094 gate, 403
...
- 89be40c: feat(objectql,runtime,lint): tighten `views:` to the declared container-only contract; assembled manifests travel non-container view artifacts in `viewItems:` (#5320, #8070)
...
- cdbffca: fix(lint): judge a schema-bound metadata form at its own binding layer (#7815)
