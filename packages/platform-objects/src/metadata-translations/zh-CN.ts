  ...
  metadataForms: {
    ...
    approval: {
      ...
      fields: {
        ...
        object: { label: '所属对象', helpText: '需要审批的对象' },
        objectName: { label: '所属对象', helpText: '需要审批的对象' },
        entryCriteria: { label: '进入条件', helpText: 'CEL 表达式：满足时用户可提交' },
        ...
        approvalStatusField: { label: '审批状态字段', helpText: '用于镜像审批状态的字段名（如 "approval_status"）' },
        statusField: { label: '审批状态字段', helpText: '用于镜像审批状态的字段名（如 "approval_status"）' },
        lockRecord: { label: '锁定记录', helpText: '审批待处理期间锁定记录，禁止编辑' },
        escalation: { label: '升级策略', helpText: '超时后自动升级或自动通过' },
        ...
        onFinalApprove: { label: '通过后动作', helpText: '所有步骤通过后的动作（如更新状态）' },
        onFinalReject: { label: '拒绝后动作', helpText: '被拒绝后的动作（如通知提交人）' },
