...
(`@objectstack/service-i18n`) implements translation bundles, locale resolution,
ICU MessageFormat, and number/date formatting — see the
[service README](/packages/services/service-i18n/README.md) for the current API.
The nested config shapes shown below (e.g. `i18n.translationService.provider`,
auto-translate hooks) describe target behaviour and are not yet codified as
top-level `defineStack` keys. Treat the snippets as design intent.
</Callout>
