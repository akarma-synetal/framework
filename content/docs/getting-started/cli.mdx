...
```bash
os start                                                    # Loads ./dist/objectstack.json
os start --port 8080                                        # Pick a port
OS_ARTIFACT_PATH=./build/myapp.json os start                # Custom artifact path
OS_ARTIFACT_PATH=https://cdn.example.com/app.json os start  # Remote artifact (HTTP/HTTPS)
```

**Options:**
- `-p, --port <port>` — Server port
- `--ui` — Enable Studio UI at `/_studio/`
- `-v, --verbose` — Verbose output

**How it resolves the artifact:**
1. `OS_ARTIFACT_PATH` env var — accepts a local file path **or** an `http(s)://` URL
2. `<cwd>/dist/objectstack.json` — the canonical default

**What it boots:**
- Reads the artifact's `manifest`, `objects`, `views`, `flows`, …
- Auto-registers the platform services declared in `requires: [...]` (e.g. `ai`, `automation`, `analytics`)
- Picks a storage driver from `OS_DATABASE_URL` (defaults to in-memory if unset)
- Runs `standalone` boot mode by default (no control plane, no auth) — set `AUTH_SECRET`
  and use a host config (`apps/objectos`-style) if you need single- or multi-project mode.
