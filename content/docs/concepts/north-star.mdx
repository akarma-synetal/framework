...
- **Open Core** - the framework, runtime, protocols, SDKs, tooling, docs, examples, and adapters are Apache-2.0 by default; only official services and plugins under `packages/services/*/` and `packages/plugins/*/` are BUSL-1.1 with a four-year conversion to Apache-2.0; hosted cloud features and marketplace terms remain separate commercial surfaces
