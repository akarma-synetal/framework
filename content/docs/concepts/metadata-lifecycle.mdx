...
`MetaRef = (type, name, org, project, branch)`. As of [ADR-0006 v4](/adr/0006-project-environment-split-v4), `project` is **dead in the runtime tuple** — only `(type, name, org, branch)` is meaningful. The Zod schema still exposes `project` for backward compatibility, defaulting to `"default"`.
