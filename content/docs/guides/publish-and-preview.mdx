...
To switch to S3 in production, register the storage service in your
...
plugins: [
  StorageServicePlugin({
    adapter: 's3',
    s3: { bucket: 'objectstack-artifacts', region: 'us-east-1', /* … */ },
  }),
  // … cloud plugins
]
```

No other code changes are required — `cloud-artifact-api-plugin` will pick it
up automatically.
